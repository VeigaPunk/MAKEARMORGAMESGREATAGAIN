...
**r09:** `hardest/` corpus **complete at 96/96 PASS** — waves 1–3 committed `9336a53`+`7e829dd` (DD-104); `node hardest/validate.mjs` re-run ~30s. Forge burn still uncommitted (DD-94); D-35 sub-frame taps still open in-tree. `prototypes/MECHANICS-DIGEST.md` §2 carries the D-33/D-34 collision lessons as build rules — already consumed by the in-tree fixes.

**r10:** `hardest/` corpus grew to **98/98 PASS** (re-verified live ~35s) — new **movers** mechanic (solid sliding blocks; push player, pinned = crushed) + levels `97-moving-walls`/`98-crush-alley`, all **uncommitted**; `manifest.js` stale at 96 → browser loads 96, validator sees 98 (DD-107). `LEVEL-FORMAT.md` documents the mover contract (w/h 1–4, speed ≤140, sweep never touches `#`/`S`/`K`). README references `pars.js`/`gen-pars.mjs`/`difficulty.mjs`/`DIFFICULTY.md` — not yet landed.

**r12:** burn **committed `69f9b24`** — `__proto` getter hook (D-22 probe hazard), gap-kill + front-edge side-kill (D-33/34, live-verified pre-commit AND re-proven post-commit in r05 PNGs x=1410/x=2967). D-35 sub-frame taps still open. `hardest/` corpus now **114 levels uncommitted** — live validator **112/114 PASS** (`109-vault-door`+`111-portal-press` autopilot sim-budget fails); `manifest.js`+`pars.js` regenerated to 114, **`DIFFICULTY.md` stale at 98** (DD-109). Proto r6 `cbd0096` added practice checkpoints (4 flags) + input-offset calibration + full-clear re-proof x=9902 (DD-110).
