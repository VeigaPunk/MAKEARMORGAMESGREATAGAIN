...
**r08:** no new impossible drift — D-33/D-34 in-tree fixes + D-35 open stand as r07 (DD-96); forge burn still uncommitted (DD-94). `hardest/` corpus grew to **32 levels** uncommitted — L31 'Keymaster' introduces `'y'` keys opening ALL `'D'` doors, new level-format vocabulary possibly undocumented in committed `LEVEL-FORMAT.md` (DD-98); re-run `node hardest/validate.mjs` → 32/32 PASS.
