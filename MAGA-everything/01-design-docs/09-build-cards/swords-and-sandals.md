...
**r08 (uncommitted, DD-94/DD-100):** sas WIP fixes landed in-tree — `validSave` + hub/complete boot (verify D-23), `points=0` on save-load (D-27), `owned[]` persisted + re-buy disabled (D-28), Buckler gate L3→L2 (D-29), `g.name` via `textContent` (D-31). **Still open:** D-30 complete-screen replay exploit (complete→hub→startFight still loops `18+defeated*8` gold). Sibling surface: `prototypes/swords-and-sandals.{html,md}` committed `550e4c9` — headless-verified loop slice, all combat constants DECLARED GUESSES per spec §Open questions; found+fixed a result-screen softlock; 9 carry-forward recs (state-machine exits, guaranteed-legal TAUNT/HOLD, stats-as-data roster, render-time shop gates, save-on-transition, stamina dial). Card awaits verify cross-verification.

**r09:** burn still uncommitted (same 11 files, DD-94); D-30 replay exploit still open in-tree. `prototypes/MECHANICS-DIGEST.md` §4 sharpens the carry-forward: "monorepo must replicate all three guards" — `load()` restores stats verbatim (D-27 ✓ in-tree), `owned.includes(i)` guard (D-28 ✓ in-tree), `nextOpp` advances past champion (D-30 ✗ still open). 2 of 3 landed; the third is the remaining exploit.

**r12:** burn **committed `69f9b24`** — all six defect fixes landed: `validSave()` schema + mode restore (D-23), `points=0` (D-27), `owned[]` persisted + re-buy disabled (D-28), Buckler gate L3→L2 (D-29), **D-30 replay exploit closed** (hub hides "Next Opponent" at `defeated≥4`; `win()` routes to `'complete'` with no replay path), `g.name`→`textContent` (D-31 XSS). Plus **3 portrait SVGs** (`portrait-azure/gold/scarlet.svg`) + canvas arena scene (dusk sky/colosseum/crowd/sand). All committed-but-unverified pending verify re-probe (DD-111); r05 SasD23/D29-30/D31 PNG probes already shot against the fixes (DD-112).
