...
**r08 (uncommitted, DD-94/DD-99):** cluck `enemyTypes`/`bosses` now drive per-type speed (`motionT` scaling) + per-type colors in render — DD-87 structure fully consumed. Shared shmup-core open items apply here too: D-36 wave-count clamp, D-37 boss-name render, D-39 hardcoded pickup/comb/beak visuals. Re-verify after commit.

**r09:** spec 06 acceptance #4 "mobile one-thumb layout clears one wave" now has a **proven reference implementation** — proto r5 `b8ce8b6` layout B (relative drag + auto-fire + edge missile button; DD-105). `MECHANICS-DIGEST.md` §3 input map applies to this pack identically (spec 06 §Mobile "same layouts A/B"). Shared shmup-core open items unchanged (D-36/37/39 + D-38 replica stats).

**r10 (uncommitted, DD-108):** the shared proto file `chicken-invaders.html` gained **layout A twin-thumb** (left-half stick + right-half fire, `KeyL` toggle, persisted `ci_proto_layout`, default 'A') — spec 06 §Mobile "same layouts A/B" means this pack inherits the same reference; still unproven (no touch clear recorded). Shared shmup-core open items unchanged (D-36/37/39 + D-38 replica stats).

**r12:** burn **committed `69f9b24`** — cluck pack `enemyTypes`×3 (FLOCKBIRD/GLIDER/BRUISER, differentiated speed/hp) + `bosses`×2 (MOTHER GOOSE/ROOSTER REGENT) + **7 authored SVGs** (`enemy-bruiser/flockbird/glider`, `boss-mother-goose/rooster-regent`, `ship-courier`, `title-cluck-horizon`) committed. Shared shmup-core open items remain: D-36 wave clamp, D-37 boss-name render, D-39 hardcoded strings. Proto r6 layout A PROVEN — spec 06 §Mobile "same layouts A/B" inherits the reference (DD-110).
