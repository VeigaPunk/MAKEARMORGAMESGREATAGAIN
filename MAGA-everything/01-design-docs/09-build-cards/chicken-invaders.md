...
**Posture:** INTERNAL-NO-PUBLIC (above). **Skeleton:** `packages/shmup-core` shared with #6 Cluck Horizon (shipped `50665ba`, proto's normalized vocabulary — DD-34 resolved-in-code; spec docs still need the vocabulary decision). **r04 live (verify):** virtual stick sticks after pointerup outside canvas — `touch.ts:84-90` canvas-only listeners (DD-82); proto `chicken-invaders.html` now verified PASS (D-21 fixed). **r06 WIP (uncommitted, DD-94):** pointerup/cancel moved to `window` (DD-82 fix); `PACKS.replica` now defines `enemyTypes`×3 + `bosses`×2 with per-type speed/hp consumed by `sim.ts`/`render.ts` (DD-87-class structure). **Gates:** G0–G5 per STACK §3.

**r07:** proto proof `prototypes/chicken-invaders.html` + new claim card `chicken-invaders.md` committed `aa15f76`. Proto-verdicts r04 (uncommitted): `__proto.wavesTotal` title-screen throw re-confirmed open (verify D-22, `prototypes/chicken-invaders.html:479-482` — DD-97); claim card postdates the verdict — itself unverified.

**r08 (uncommitted, DD-94/DD-99):** replica `enemyTypes`×3 now consumed per-type in `sim.ts` (`motionT` speed scaling) + `render.ts` (variant colors) — but the three replica types remain stat-identical (verify D-38 stands); boss names still unrendered (verify D-37); HUD `WAVE 3/2` unclamped post-final-wave (verify D-36); pack gift/food strings + comb/beak hardcoded (verify D-39). All open in-tree.

**r09:** proto r5 `b8ce8b6` — **mobile layout B (one-thumb) proven touch-only** (spec 04 §Mobile B): relative drag moves ship, auto-fire on, 64px MISSILE edge button; wave 1 cleared touch-only (DD-105). `__proto.wavesTotal` title-screen throw FIXED (verify D-22 closed). `MECHANICS-DIGEST.md` §3 has the proven input map; layout A (twin-thumb) still unproven. Forge-side open items unchanged (D-36/37/38/39 in-tree).

**r10 (uncommitted, DD-108):** proto `chicken-invaders.html` now implements **layout A twin-thumb** — left-half virtual stick + right-half hold-fire (`firePtrs`), missile button raised to `y=H-172`, `KeyL` title toggle, `ci_proto_layout` localStorage, **default 'A'** (the spec's recommended layout, spec 04 §Controls A/§Mobile A). Built but unproven — no claim-card touch clear yet; `MECHANICS-DIGEST.md` §6 "unbuilt" is stale. Forge-side open items unchanged (D-36/37/38/39 in-tree).

**r12:** burn **committed `69f9b24`** — replica `enemyTypes`×3 + `bosses`×2 structure committed with per-type motion/colors; **but replica's three CHICKEN types remain stat-identical (D-38 open)**, boss names unrendered (D-37), `WAVE n/2` unclamped (D-36), pack gift/food strings + comb/beak hardcoded (D-39). Proto r6 `cbd0096`: **layout A twin-thumb PROVEN** — CDP multi-touch wave-1 clear score 1200 (spec 04 §Mobile A + acc #5); both spec'd mobile layouts now proven (DD-110, DD-108 RESOLVED).
